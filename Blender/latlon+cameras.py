...
if not moon:
    ...
    radius = 1  # Moon yarıçapı
    altitude = 1.0  # Yüzeyden yükseklik

    # Eski kameraları sil (isimleri Camera_Lat_ ile başlayanlar)
    for obj in bpy.data.objects:
        if obj.name.startswith("Camera_Lat_"):
            bpy.data.objects.remove(obj, do_unlink=True)

    # Her 10 derecede bir kamera ekle
    for lat in range(0, 81, 10):
        lat_rad = math.radians(lat)
        lon_rad = math.radians(0)  # 0. boylam

        total_distance = radius + altitude
        x = total_distance * math.cos(lat_rad) * math.cos(lon_rad)
        y = total_distance * math.cos(lat_rad) * math.sin(lon_rad)
        z = total_distance * math.sin(lat_rad)
        cam_position = Vector((x, y, z))
        ...
        # Kamera oluştur
        cam_data = bpy.data.cameras.new(f"Camera_Lat_{lat}")
        cam_obj = bpy.data.objects.new(f"Camera_Lat_{lat}", cam_data)
        bpy.context.collection.objects.link(cam_obj)

        cam_obj.location = cam_position

        # Ay merkezine bakacak şekilde yönlendir
        direction = moon.location - cam_position
        direction.normalize()
        cam_obj.rotation_mode = 'QUATERNION'
        cam_obj.rotation_quaternion = direction.to_track_quat('-Z', 'Y')

        print(f"Lat {lat}° → Kamera yerleştirildi.")
